...
def make_resource(resource_id, proc_id, node_name, cluster,
...
def main():
    ...
    for proc_id in range(max(all_procs) + 1):
        ...
        resources.append(
            make_resource(rid, proc_id, node_name, args.cluster,
                          core_in_node, cpn)
        )
    ...
    for row in rows:
        ...
        jobs[job_id] = {
            ...
            "array_index":          1,
            "resubmit_job_id":      0,
            "cpuset_name":          f"{workload}_{job_id}",
            "launching_directory":  "",
